...
Most common and effective method of providing reliable services is replication. Applying the replication requires only minor changes between non-replicated services and the replicated ones, what makes it usable both for creating new services in the typical way % TODO: poprawić zwrot
...
The crash-recovery has been also the subject among the articles. The \cite{rodriguez2000atomic} provides definitions concerning recovery as well as shows method to transform any crash-stop consensus protocol to crash-recovery one.
