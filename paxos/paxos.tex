...
\begin{description}
  ...
  so it proposes no-op for instance $i$. % TODO: TZ why?
...
But the rule above % TODO JK what rule
...
% TODO JK what is snapshotting (write one - two sentences of introduction)
...
So in JPaxos each \propose is simultaneously an \accept message. Notice that if the system consists of three nodes every process decides on the value by receiving a valid propose -- he knows that the leader and it itself % TODO TZ ??
...
Most of the articles about Paxos state that the \accept message carries the value proposed by proposer. %TODO: TZ
...
Waiting for the \propose message might seem to cause additional overhead, but it makes JPaxos faster -- less network bandwidth is used by a single instance. % TODO JK why? explain
...
% TODO JK Value - value to be agreed upon?
% TODO JK what are the numbers inside table the number of messages sent
...
Other implementations consider using bounded learner count % TODO JK can't understand what you mean
